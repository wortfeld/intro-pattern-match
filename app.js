...
import { ensureFFmpeg, decodeHeadToWav, decodeSegmentToWav, fetchHeadUrl, fetchFullUrl, bufferToFileLike, getDurationFromFile, getDurationFromSrc } from './media.js';
...
import { parseTime, fmtMMSS, fmtHHMMSS, basenameFromUrl, parseMetaCsv, uuid } from './meta.js';
...
const targetUrlsEl = document.getElementById('targetUrls');
const btnAnalyzeUrls = document.getElementById('btnAnalyzeUrls');
const metaCsvEl = document.getElementById('metaCsv');
...
btnAnalyze.addEventListener('click', async ()=>{
  try{
    ...
    const metaMap = parseMetaCsv(metaCsvEl.value);
    ...
    for(let i=0;i<files.length;i++){
      ...
      const [buf, duration_s] = await Promise.all([
        ...
        (Number.isFinite(metaMap[file.name]?.duration_s) ? Promise.resolve(metaMap[file.name].duration_s) : getDurationFromFile(file).catch(()=>NaN))
      ]);
      const wav = parseWavPCM16(buf);
      if(wav.sampleRate!==16000) log('note: sampleRate='+wav.sampleRate);
...
btnAnalyzeUrls.addEventListener('click', async ()=>{
  try{
    await ensureCore();
    const lines = (targetUrlsEl.value||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
    if(lines.length===0){ log('enter at least one URL'); return; }
    const key = patternSelect.value; if(!key || key.startsWith('(no')){ log('select a pattern'); return; }
    const headWindow = parseInt(headWindowEl.value,10)||180;
    const frameSize = parseInt(frameSizeEl.value,10)||1024;
    const hop = parseInt(hopSizeEl.value,10)||512;
    ...
    for(const u of lines){
      ...
      const durationPromise = Number.isFinite(meta.duration_s) ? Promise.resolve(meta.duration_s) : getDurationFromSrc(u).catch(()=>NaN);

      const obj = await fetchHeadUrl(u, 64*1024*1024);
