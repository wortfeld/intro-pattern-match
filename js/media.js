...
function toBlobURL(url, mime){
  return fetch(url).then(r=>{ if(!r.ok) throw new Error('fetch '+r.status+' '+url); return r.arrayBuffer(); })
...
export function bufferToFileLike(buf, name){ return { name, arrayBuffer: () => Promise.resolve(buf) }; }

export function fetchHeadUrl(url, maxBytes=64*1024*1024){
  return fetch(url, { headers: { 'Range': 'bytes=0-'+(maxBytes-1) }, mode: 'cors' })
    .then(async r => {
...
export function fetchFullUrl(url, maxMB=300){
  return fetch(url, { mode: 'cors' }).then(async r => {
    if(!r.ok) throw new Error('HTTP '+r.status+' '+url);
    const len = parseInt(r.headers.get('Content-Length')||'0',10);
    if(len && len > maxMB*1024*1024) throw new Error('file too large ('+len+' bytes) for cap '+maxMB+' MB');
    const buf = await r.arrayBuffer();
    return { name: url, buffer: buf };
  });
}

export function getDurationFromFile(file){
  const url = URL.createObjectURL(file);
  return getDurationFromSrc(url).finally(()=>URL.revokeObjectURL(url));
}
export function getDurationFromSrc(src){
  return new Promise((resolve,reject)=>{
    const el = document.createElement('video');
    el.preload='metadata'; el.crossOrigin='anonymous';
    el.onloadedmetadata = () => { const d = el.duration; el.removeAttribute('src'); el.load(); Number.isFinite(d)? resolve(d) : reject(new Error('duration unavailable')); };
    el.onerror = () => { el.removeAttribute('src'); el.load(); reject(new Error('metadata load failed')); };
    el.src = src;
  });
}
